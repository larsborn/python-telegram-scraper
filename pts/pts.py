...
def main():
    ...
    parser.add_argument('--nsqd-port', default=os.environ.get('NSQ_WRITE_PORT', 4151))
        ...
        if args.command == 'crawl_channel':
            ...
            for response in scraper.crawl(args.channel_name):
                ...
                for post in scraper.posts(response):
                    ...
                    if last_crawl_timestamp is not None \
                            and post.raw.post_datetime < pytz.UTC.localize(last_crawl_timestamp):
                        break_crawl = True
                        break
